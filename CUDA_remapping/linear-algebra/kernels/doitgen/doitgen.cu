...
{
	int p = remappingBlockIDx(blockIdx.x, BLOCKXMODE) * blockDim.x + remappingThreadIDx(threadIdx.x, THREADXMODE);
	int q = remappingBlockIDy(blockIdx.y, BLOCKYMODE) * blockDim.y + remappingThreadIDy(threadIdx.y, THREADYMODE);
