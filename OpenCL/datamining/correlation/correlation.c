...
void cl_launch_kernel(int m, int n, DATA_TYPE POLYBENCH_2D(data,M,N,m,n))
{
	...
	globalWorkSize[0] = (size_t)ceil(((float)M) / ((float)DIM_LOCAL_WORK_GROUP_KERNEL_1_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_1_X;
	globalWorkSize[1] = 1;
	...
	for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
            ...
            coalescing[0] = cX;
            coalescing[1] = cY; 
            errcode |= clSetKernelArg(clKernel_mean, 5, sizeof(int), (void *)&cX);
            errcode |= clSetKernelArg(clKernel_mean, 6, sizeof(int), (void *)&cY);
			...
			for (int i = 0; i < iter; i++) {
				errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_mean, 1, NULL, globalWorkSizeC, localWorkSize, 0, NULL, NULL);
    ...
    globalWorkSize[0] = (size_t)ceil(((float)M) / ((float)DIM_LOCAL_WORK_GROUP_KERNEL_2_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_2_X;
    globalWorkSize[1] = 1;
	...
	for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
            ...
            coalescing[0] = cX;
            coalescing[1] = cY;
            errcode |= clSetKernelArg(clKernel_std, 7, sizeof(int), (void *)&cX);
            errcode |= clSetKernelArg(clKernel_std, 8, sizeof(int), (void *)&cY);
            if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
			...
			// Execute the OpenCL kernel
			for (int i = 0; i < iter; i++) {
				errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_std, 1, NULL, globalWorkSizeC, localWorkSize, 0, NULL, NULL);
    ...
    globalWorkSize[0] = (size_t)ceil(((float)M) / ((float)DIM_LOCAL_WORK_GROUP_KERNEL_3_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_3_X;
	...
	for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
            ...
            coalescing[0] = cX;
            coalescing[1] = cY;
            errcode |= clSetKernelArg(clKernel_reduce, 6, sizeof(int), (void *)&cX);
            errcode |= clSetKernelArg(clKernel_reduce, 7, sizeof(int), (void *)&cY);
            if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
			...
			for (int i = 0; i < iter; i++) {
				...
				errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_reduce, 2, NULL, globalWorkSizeC, localWorkSize, 0, NULL, NULL);
	...
	printf("finished kernel 3\n");

	/* Kernel 4 */
	localWorkSize[0] = DIM_LOCAL_WORK_GROUP_KERNEL_4_X;
    localWorkSize[1] = DIM_LOCAL_WORK_GROUP_KERNEL_4_Y;
    globalWorkSize[0] = (size_t)ceil(((float)M) / ((float)DIM_LOCAL_WORK_GROUP_KERNEL_4_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_4_X;    
	globalWorkSize[1] = 1;
    ...
    // Execute the OpenCL kernel
	//for (int i = 0; i < iter; i++) {
    	errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_corr, 1, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
	...
	for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
            ...
            coalescing[0] = cX;
            coalescing[1] = cY;
            errcode |= clSetKernelArg(clKernel_corr, 4, sizeof(int), (void *)&cX);
            errcode |= clSetKernelArg(clKernel_corr, 5, sizeof(int), (void *)&cY);
            if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
            ...
            for (int i = 0; i < iter; i++) {
				// Execute the OpenCL kernel
				errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_corr, 1, NULL, globalWorkSizeC, localWorkSize, 0, NULL, NULL);
				//clEnqueueBarrier(clCommandQue);
				if(errcode != CL_SUCCESS) printf("Error in launching kernel4\n");
				clEnqueueBarrier(clCommandQue);
				//DATA_TYPE val = 1.0;
...
{	
	...
	cl_launch_kernel(m, n, POLYBENCH_ARRAY(data));
