...
void cl_launch_kernel(int m, int n, DATA_TYPE POLYBENCH_2D(data,M,N,m,n))
{
	...
	for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
            ...
            coalescing[0] = cX;
            coalescing[1] = cY;
            errcode |= clSetKernelArg(clKernel_reduce, 4, sizeof(int), (void *)&cX);
            errcode |= clSetKernelArg(clKernel_reduce, 5, sizeof(int), (void *)&cY);
            if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
	...
	// Execute the OpenCL kernel
	errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_covar, 1, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
