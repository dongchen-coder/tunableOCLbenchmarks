...
void cl_launch_kernel(int ni, int nj, int nk, DATA_TYPE alpha, DATA_TYPE beta)
{
  ...
  int iter = 10;
    ...
    for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
        ...
        coalescing[0] = cX;
        coalescing[1] = cY;
        errcode |= clSetKernelArg(clKernel, 8, sizeof(int), (void *)&cX);
        errcode |= clSetKernelArg(clKernel, 9, sizeof(int), (void *)&cY);
        if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
        ...
        polybench_start_instruments;
  			...
  			for (int i = 0; i < iter; i++) {
					errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel, 2, NULL, globalWorkSizeC, localWorkSize, 0, NULL, NULL);
