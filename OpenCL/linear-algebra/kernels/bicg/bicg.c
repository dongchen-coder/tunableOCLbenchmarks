...
void cl_launch_kernel(int nx, int ny)
{
	...
	for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
            ...
            coalescing[0] = cX;
            coalescing[1] = cY;
            errcode |= clSetKernelArg(clKernel1, 5, sizeof(int), (void *)&cX);
            errcode |= clSetKernelArg(clKernel1, 6, sizeof(int), (void *)&cY);
            if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
	...
	for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
            ...
            coalescing[0] = cX;
            coalescing[1] = cY;
            errcode |= clSetKernelArg(clKernel2, 5, sizeof(int), (void *)&cX);
            errcode |= clSetKernelArg(clKernel2, 6, sizeof(int), (void *)&cY);
            if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
