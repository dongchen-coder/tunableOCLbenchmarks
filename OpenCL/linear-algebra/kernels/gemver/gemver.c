...
void cl_launch_kernel(int n, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE POLYBENCH_2D(A,N,N,n,n))
{
	...
	for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
		for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
			...
			for (int i = 0; i < iter; i++) {
				...
				errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel1, 2, NULL, globalWorkSizeC, localWorkSize, 0, NULL, NULL);
    ...
    coalescingMax[0] = globalWorkSize[0] / localWorkSize[0];
    coalescingMax[1] = globalWorkSize[1] / localWorkSize[0];
    iter = 10;
    ...
    for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
			...
			// Execute the OpenCL kernel	
			for (int i = 0; i < iter; i++) {
				errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel2, 1, NULL, globalWorkSizeC, localWorkSize, 0, NULL, NULL);
    ...
    for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
            ...
            coalescing[0] = cX;
            coalescing[1] = cY;
            errcode |= clSetKernelArg(clKernel3, 5, sizeof(int), (void *)&cX);
            errcode |= clSetKernelArg(clKernel3, 6, sizeof(int), (void *)&cY);
            if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
			...
			// Execute the OpenCL kernel
			for (int i = 0; i < iter; i++) {
				errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel3, 1, NULL, globalWorkSizeC, localWorkSize, 0, NULL, NULL);
...
int main(int argc, char *argv[])
{
	...
	cl_load_prog();
	cl_launch_kernel(n, alpha, beta, POLYBENCH_ARRAY(A));
