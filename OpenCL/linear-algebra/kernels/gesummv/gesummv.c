...
void cl_launch_kernel(int n, DATA_TYPE alpha, DATA_TYPE beta)
{
    ...
    coalescingMax[1] = globalWorkSize[1] / globalWorkSize[1];
    int iter = 10;
    ...
    for (int cX = 1; cX <= coalescingMax[0]; cX = 2*cX) {
        for (int cY = 1; cY <= coalescingMax[1]; cY = 2*cY) {
            ...
            coalescing[0] = cX;
            coalescing[1] = cY;
            errcode |= clSetKernelArg(clKernel1, 8, sizeof(int), (void *)&cX);
            errcode |= clSetKernelArg(clKernel1, 9, sizeof(int), (void *)&cY);
