...
void cl_launch_kernel(int n)
{
	...
	size_t coalescingMax;
	coalescingMax = globalWorkSize[0] / localWorkSize[0];
	int iter = 10;

	for (int cX = 1; cX <= coalescingMax; cX = 2*cX) {
        int coalescing = cX;
        errcode |= clSetKernelArg(clKernel1, 4, sizeof(int), (void *)&cX);
        if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
		...
		for (int i = 0; i < iter; i++) {
			errcode = clEnqueueWriteBuffer(clCommandQue, x1_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * N, NULL, 0, NULL, NULL);
			errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel1, 1, NULL, globalWorkSizeC, localWorkSize, 0, NULL, NULL);
			clFinish(clCommandQue);
			if(errcode != CL_SUCCESS) printf("Error in launching kernel\n");
		}

		printf("Work group number %lu 1\n", coalescingMax/cX);
	...
	for (int cX = 1; cX <= coalescingMax; cX = 2*cX) {
        int coalescing = cX;
        errcode |= clSetKernelArg(clKernel2, 4, sizeof(int), (void *)&cX);
        if(errcode != CL_SUCCESS) printf("Error in seting arguments\n");
